...
def test_docs_plugins_controller_interface_example(tmp_path: Path):
    """docs: check controller interface example plugin works"""
...
@pytest.mark.cluster_plugin
def test_docs_plugins_controller_helpers_loop_example(tmp_path: Path):
    """docs: check controller helpers LoopController example works"""
    base = Path(__file__).parent.parent.parent.parent
    example_file = base / "docs" / "plugins" / "controller" / "helpers.md"
    example_md = example_file.read_text()
