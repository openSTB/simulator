...
# plugins from the tests/plugin/modules/registered and tests/plugin/modules/installed
# directories. These will be available under the openstb_simtestplugin.registered and
# openstb_simtestplugin.installed namespaces, respectively. Each sub-module in the
# former will also be registered as an entry point for the
# "openstb.simulator.testplugin" group with the name specified in a comment at the top
# of the file.
...
def test_plugins():
    ...
    yield Path(__file__).parent / "modules" / "local"
...
def test_plugin_registered_empty():
    """plugin.loader.registered_plugins: group with no plugins"""
...
def test_plugin_registered(test_plugins):
    """plugin.loader.registered_plugins: check basic behaviour"""
    registered = loader.registered_plugins("openstb.simulator.testplugin")
...
def test_plugin_load_class(test_plugins):
    """plugin.loader.load_plugin_class: check basic behaviour"""
...
def test_plugin_load_class_missing(test_plugins):
    """plugin.loader.load_plugin_class: check with invalid plugin name"""
    ...
    with pytest.raises(ValueError, match="could not import plugin"):
...
def test_plugin_load_class_multi(test_plugins):
    """plugin.loader.load_plugin_class: multiple registered plugins of same name"""
...
def test_plugin_load_class_local_err(test_plugins):
    """plugin.loader.load_plugin_class: local plugins with errors"""
...
def test_plugin_load_spec(test_plugins):
    """plugin.loader.load_plugin: check with spec dictionary"""
